...
![embeddings](../results/05_siamese_contrastive_mnist/embeddings_pca.png)
![curves](../results/05_siamese_contrastive_mnist/training_curves.png)
...
- **Contrastive vs. triplet loss** → see [project 06](../06_siamese_triplet_resnet).
- `sqrt(max(x, ε))` avoids the infinite gradient of √0.

## Run
```bash
python 05_siamese_contrastive_mnist/train.py   # ~2 min on a T4
```
