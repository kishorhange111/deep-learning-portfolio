...
- **Fixes on small data:** stronger augmentation (RandAugment, Mixup/CutMix), DeiT-style distillation from a CNN teacher, smaller patches, hybrid conv-stem, or start from a pre-trained ViT and fine-tune. [Project 02 (ShiftViT)](../../computer-vision/shiftvit-attention-free-transformer) shows a hierarchical design that works better on small images.
...
## Run
```bash
python computer-vision/vision-transformer-cifar100/train.py                 # 40-min budget
TIME_LIMIT_MIN=120 EPOCHS=100 python computer-vision/vision-transformer-cifar100/train.py
```
